...
const lyricstranslateAPI = {
  getLyrics: async (nid) => {
    return await fetch('https://lyricstranslate.com/en/callback/ltlyricsondemand/get/lyrics', {
      headers: {
  ...
  getTransliteration: async (customId) => {
    return await fetch('https://lyricstranslate.com/en/callback/transliteration', {
      headers: {
        accept: 'application/json, text/javascript, */*; q=0.01',
        'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'sec-fetch-dest': 'empty',
...
async function getData(url) {
  ...
  if (!data.lyricsDownloaded) {
    ...
    for (song of data.songs) {
      // Original song
      console.debug(`   Dowloading lyrics for song ${getURLPageDecode(song.link)} ...`)
      ...
      // );
      const transliterationData = await getTransliterationFromId(song.transliteration.id)
      // console.debug(
      ...
      for (let englishTranslation of song.englishTranslations) {
        ...
        // );
        const englishTranslationData = await getLyricsFromId(englishTranslation.id)
        // console.debug(
...
async function getLyricsFromId(id) {
  ...
  if (lyricsData.status !== 1) {
    console.warn(`Failed to get lyrics for id ${id}: ${JSON.stringify(lyricsData)}`)
    return data
  }
...
async function getTransliterationFromId(id) {
  ...
  if (transliterationData === null) {
    console.warn(`Failed to get transliteration lyrics for id ${id}: ${JSON.stringify(transliterationData)}`)
    return data
  }
...
function extractVerses(html) {
  ...
  let verses = Array.from(lyricsBody.querySelectorAll('[class^="ll-"]')).map((div) => ({
...
async function extractSongsFromArtistPage(url) {
  ...
  for (let song of songs) {
    ...
    console.debug(`   Extracting IDs for song ${getURLPageDecode(song.link)} ...`)

    for (let englishTranslation of song.englishTranslations) {
      const link = englishTranslation.link
      // Get IDs
      console.debug(
        `      Extracting IDs for english translation ${getURLPageDecode(link)} of song ${getURLPageDecode(song.link)} ...`
      )
      ...
      if (song.id === null) {
        ...
        if (IDs.songId !== song.id) console.warn(`      Ignoring song ID ${IDs.songId}: Mismatch (${song.id}) for ${link} .`)
      ...
      const englishTranslationsId = song.englishTranslations.map((englishTranslation) => englishTranslation.id)
...
function extractSongsData(document) {
  let songsData = Array.from(document.querySelectorAll('.songName')).map((td) => {
    ...
    return {
      ...
      englishTranslations: extractEnglishLinks(td.nextElementSibling).map((link) => ({ link: decodeURI(link.href) })),
    }
  })
...
function extractEnglishLinks(td) {
  ...
  // Get single link
  let link = Array.from(td.querySelectorAll('a')).filter((a) => a.textContent === 'English')[0]
  if (link !== undefined) return [link]

  // Get multiple links
  let english = Array.from(td.childNodes).filter(
    (node) => node.nodeType === Node.TEXT_NODE && node.textContent.includes('English')
  )[0]
